...
def main():
    ...
    def kMeans(data, clusters, metric='softdtw', plot=False, saveDirectory="./.figs"):
        ...
        if(plot): 
            ...
            fig.tight_layout()
            plt.savefig(os.path.join('./.figs/TSK-Means-AE-real-K-'+ str(k) + currentTime))
            # plt.show()
    ...
    dataset = random.sample(dataset, 100000)
    ...
    for k in range(2,30):
        sil_score, cluster_center = kMeans(x_train, k, metric='dtw', plot=True)
        ...
        for c in cluster_center:
            plt.plot(c)
        plt.savefig(f'./.figs/tsk-means_clusters_K_{k}_{currentTime}')
