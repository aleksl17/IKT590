...
def main():
    ...
    def cluster(x, reduction, k = 3, figDir='./.figs/'):
        ...
        print(max(ahc_pred))
        return silhouette_score(x, ahc_pred), max(ahc_pred)
    ...
    def elbow_method_t(x, reduction):
        thresholds = range(5,55,5)
        thresholds = np.asarray(thresholds)/10
        scores = []
        n = []
        for k in thresholds:
            sil, max = cluster(x, reduction, k)
            scores.append(sil)
            n.append(max)
