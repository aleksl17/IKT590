...
def main():
    ...
    def kMeans(data, clusters, metric='dtw', plot=False, saveDirectory="./.performance_score"):
        sz = data.shape[1]
        data = data[:100]
        ...
        sil = silhouette_score(data, y_pred, metric=metric)#, model.cluster_centers_
        cal = calinski_harabasz_score(data, y_pred)#, model.cluster_centers_
        dav = davies_bouldin_score(data, y_pred)#, model.cluster_centers_
        
        
        return sil, cal, dav, model.cluster_centers_
    ...
    for k in range(2,10):
        sil_score, cal_har_score, dav_bou_score, cluster_center = kMeans(x_train, k, metric='softdtw', plot=True)
        ...
        for c in cluster_center:
            plt.plot(c)
        plt.savefig(f'./.performance_score/clusters_K_{k}_{currentTime}')

    plt.clf()   
    plt.plot(sil_scores)
    plt.plot(cal_har_scores)
    plt.plot(dav_bou_scores)

    plt.savefig(f'./.performance_score/sil_score_{currentTime}')
    plt.savefig(f'./.performance_score/cal_har_score_{currentTime}')
    plt.savefig(f'./.performance_score/dav_bou_score_{currentTime}')
    # plt.show()  
